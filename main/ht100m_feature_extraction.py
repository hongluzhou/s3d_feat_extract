...
from datasets.ht100m.ht100m_video_narration_loader import HT100M_DataLoader
...
def get_args(
    description = \
        'Feature extraction for HowTo100M using S3D model trained with MIL-NCE.'):
    
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('--video_id_csv', type=str,
                        default='csv/howto100m_videos.csv', help='video ids csv')
    parser.add_argument('--video_path', type=str,
                        default='', help='video dir')
    parser.add_argument('--caption_root', type=str,
                        default='', help='caption dir')
    ...
    parser.add_argument('--feat_save_root', type=str,
                        default='/export/home/data/howto100m/feats',
...
def main_worker(ngpus_per_node, args):
    ...
    # Data loading code
    ht100m_dataset = HT100M_DataLoader(
        args=args,
        csv=args.video_id_csv,
    ...
    data_loader = DataLoader(
        ...
        collate_fn=ht100m_dataset.custom_collate,
    ...
    feat_extract(data_loader, ht100m_dataset, model, args)
...
def feat_extract(data_loader, dataset, model, args):
    ...
    with torch.no_grad():
        for i_batch, sample_batch in enumerate(data_loader):
            ...
            captions = sample_batch['captions']
            # captions: (B, P, S, args.max_words) 
            # E.g., (2, 29, 3, 16)
            ...
            if False in vide_read_ok:  # some videos are corrupted or problematic
                ...
                captions = captions[vide_read_ok]
            ...
            if sum(vide_read_ok) > 0:
                try:
                    ...
                    clips = clips / 255.0
                    captions = captions.cuda(non_blocking=args.pin_memory)

                    # pdb.set_trace()
                    video_output, text_output  = model(
                        **dict(video=clips.flatten(0,1).flatten(0,1),  # (B', 3, 32, 224, 224) where B'=BxPxS
                        text=captions.flatten(0,1).flatten(0,1),    # (B', 16)
                        mode='all', mixed5c=False, raw_text=False))
                    ...
                    # similarity_matrix = torch.matmul(text_embedding, video_embedding.t())
                    ...
                    text_embed_np = text_embedding.view(B,P,S,-1).cpu().numpy() 
                    for v_id in range(len(video_id)):
                        ...
                        np.save(
                            os.path.join(args.feat_save_root, v_sid, 'text.npy'),
                            text_embed_np[v_id][:last_none_padding_index[v_id]]  # (actual_P, S, 512)
                        )
                        np.save(
