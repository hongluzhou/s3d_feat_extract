...
class HT100M_DataLoader(Dataset):
    ...
    def __init__(
        ...
        if 'howto100m_videos.csv' in csv:
            self.csv = pd.read_csv(csv)
        else:
            self.csv = pd.read_csv(csv, sep='\t', names=["video_path"])
            for row_index in range(len(self.csv)):
                self.csv.iloc[row_index]['video_path'] = self.csv.iloc[row_index]['video_path'].split(' ')[0]
        ...
        # self.video_files = [video_file for video_file in self.csv['video_path']]
        ...
        for video_file in self.csv['video_path']:
        ...
        # self.videos = ['drl7Trk51UA']  # ['Bi62o3FSkRI', '7U09YG1VUoY']
        # self.video_files = []
        # for video_file in self.csv['video_path']:
        #     video_id = video_file.split('.')[0]
        ...
        # 1,238,791 samples... if MIL-NCE_HowTo100M
        # 85,102 samples... 
    ...
    def _get_text_video_segments(self, video_path, caption_path, keep_raw_captions=False):
        """ Note: 
        - assuming self.clip_window is a multiple of self.num_sec 
        - self.num_sec is expected to be 3.2 (https://arxiv.org/abs/1912.06430)
        """
        
        cap = pd.read_csv(caption_path)
        
        ### load clips (with captions) of video
        start = 0
        end = start + self.clip_window
        
        clips, clip = [], []
        if keep_raw_captions:
            raw_captions, raw_caption = [], []
        captions, caption = [], []
        ...
        while not self._reach_end_of_video(possibly_end, thresh=thresh):
            ...
            if not vide_read_ok:
                ...
                captions_stacked = th.zeros((0, round(self.clip_window/self.num_sec), self.max_words), dtype=th.long)
                times_stacked = th.zeros((0, round(self.clip_window/self.num_sec)))
                raw_captions = []
                if keep_raw_captions:
                    return vide_read_ok, clips_stacked, captions_stacked, times_stacked, raw_captions
                else:
                    return vide_read_ok, clips_stacked, captions_stacked, times_stacked
                
            subclip_cap = self._get_caption(cap, start)  # raw caption of this subclip
            
            if start <= end:
                clip.append(subclip)
                if keep_raw_captions:
                    raw_caption.append(subclip_cap)
                caption.append(self.words_to_ids(subclip_cap))
                time.append(start)
        ...
        # == Obtained clips, captions, raw_captions, times.
        # == All of them are list of lists.
            
        clips_stacked, captions_stacked = [], []
        for clip_idx in range(len(clips)):
            clips_stacked.append(th.stack(clips[clip_idx]))
            captions_stacked.append(th.stack(captions[clip_idx]))
        
        # pdb.set_trace()
        if len(clips_stacked) > 0:
            clips_stacked = th.stack(clips_stacked)  # torch.Size([clip_num, subclip_num_of_each_clip, 3, 32, 224, 224])
            captions_stacked = th.stack(captions_stacked)  # torch.Size([clip_num, subclip_num_of_each_clip, args.max_words])
    ...
    def custom_collate(batch):
        ...
        captions = pad_sequence(
            [data['captions'] for data in batch], batch_first=True)
    ...
    def __getitem__(self, idx):
        ...
        vide_read_ok, clips, captions, start_times = self._get_text_video_segments(
            video_path,
            os.path.join(self.caption_root, video_id + '.csv'),
            keep_raw_captions=False)
